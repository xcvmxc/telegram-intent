...
Pipeline (subcommands driven by the /tgjobs command in whichever agent):
...
def cmd_pull(args: argparse.Namespace) -> int:
    """Scan every source; stash messages into the DB. In `links` mode only
    posts with a URL are stored; in `text`/`both` mode any text post is."""
    now = dt.datetime.now(dt.timezone.utc).isoformat()
    conn = db.connect()
    sources = load_sources()
    mode = config.load()["search_mode"]
    ...
    if not sources:
        summary["errors"].append({
            "error": f"No sources listed in {config.sources_file()}. "
                     "Add at least one channel and run /tgjobs again.",
        })
    ...
    for ref in sources:
        ...
        for m in msgs:
            ...
            if not urls:
                if mode == "links":
                    continue              # links mode: ignore link-less posts
                if not text.strip():
...
_EMIT_I18N = {
    "en": {
        ...
        "generated": "Generated by /tgjobs at {when}. Rows: {n}.",
    ...
    "ru": {
        ...
        "generated": "Сгенерировано /tgjobs: {when}. Строк: {n}.",
...
def _render_table(rows: list[sqlite3.Row], when_iso: str, lang: str = "en",
    ...
    for key in order:
        ...
        row = f"| {pos} | {co} | {links} | {date_only} | {tgs} |"
        if show_excerpt:
            row = f"{row} {_md_cell(g['excerpt'])} |"
        lines.append(row)
    lines.append("")
    return "\n".join(lines)
...
def cmd_emit_files(args: argparse.Namespace) -> int:
    ...
    if rows:
        show_excerpt = cfg.get("search_mode", "links") in ("text", "both")
        prefix = _EMIT_I18N.get(lang, _EMIT_I18N["en"])["prefix"]
        path = out_dir / f"{prefix}+{stamp}.md"
        path.write_text(_render_table(rows, now_utc, lang, show_excerpt), encoding="utf-8")

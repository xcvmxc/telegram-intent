...
каждое сообщение со ссылкой (в режиме `links`) или любой текстовый пост
(в режимах `text`/`both`) и печатает JSON-сводку.
...
**Следуй разделу «Как формируется результат» в критериях** — там указано, искать
по apply-ссылкам, по постам целиком или по обоим. Если раздел разрешает пост без
ссылки как результат — ставь `link` = `permalink` сообщения и добавляй одну
строку `excerpt` (короткую цитату из поста); должность/компанию бери из текста.
Для результатов-ссылок `link` — это apply-URL, а `excerpt` можно не указывать.
